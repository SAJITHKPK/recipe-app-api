...
class PrivateRecipeApiTests(TestCase):
    ...
    def test_update_user_returns_error(self):
        """Test changing user returns an error."""

        new_user = create_user(email="newuser@example.com", password="newuser1234")
        recipe = create_recipe(user=self.user)
    ...
    def test_delete_other_users_recipe_error(self):
        ...
        new_user = create_user(email="newuser2@example.com", password='newpass123')
    ...
    def test_update_recipe_assign_tag(self):
        """Tast assigning an existing tag when updating a recipe."""

        tag_breakefast = Tag.objects.create(user=self.user, name='Breakfast')
        recipe = create_recipe(user=self.user)
    ...
    def test_create_recipe_with_ingredients(self):
        ...
        payload = {
            ...
            'time_minutes': 100,
            'price': Decimal('4.50'),
            'ingredients': [{'name': 'Checken'}, {'name': 'tomoto'}],
        }
    ...
    def test_filter_by_tags(self):
        """testing recipes by tag."""
        r1 = create_recipe(user=self.user, title='Thai vegetarian curry')
        r2 = create_recipe(user=self.user, title='Ghee rice with Kadai')
    ...
    def test_filter_by_ingredients(self):
        ...
        r1 = create_recipe(user=self.user, title='Biriyani')
        r2 = create_recipe(user=self.user, title='chicken rice')
        in1 = Ingredient.objects.create(user=self.user, name='Tomoto')
        in2 = Ingredient.objects.create(user=self.user, name='Chicken')
        r1.ingredients.add(in1)
        r2.ingredients.add(in2)
        r3 = create_recipe(user=self.user, title='Tomoto chutney')
